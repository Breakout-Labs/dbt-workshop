...
with applications as (
    select
        *
        --* except (application_id, submitted_at),
        --id as application_id,             -- Prefer descriptive names
        --submitted_ts as submitted_at      -- "_at" suffix for timestamps
    /*
    Didn't have the staging model ready yet.
    Replace the raw table ref with dbt ref to stg_banking__loan_applications, will ya?
    Remember to adjust the select statement, the renaming is already handled in the staging model.
    Cheers, Leo
    */
    from {{ ref('stg_banking__loan_applications') }} --`breakout-labs-training`.raw_banking.loan_applications   -- TODO: Replace with dbt ref to staging
),
...
aggregated as (
    select
        ...
        coalesce(round(avg(latest_approved_amount)),0) as avg_approved_amount,
